...
const findAll = async (req, res) => {
    const { error, value } = (0, validateRequest_1.validateRequest)(productCategorySchema_1.findAllProductCategoriesSchema, req.query);
    if (error) {
    ...
    try {
        ...
        const result = await productCategoryModel_1.ProductCategoryModel.findAndCountAll({
            where: {
                deleted: 0,
                ...(Boolean(search) && {
                    productCategoryName: { [sequelize_1.Op.like]: `%${search}%` }
                })
            },
        ...
        logger_1.default.info('Product categories retrieved successfully');
